...
id_to_genotypes = defaultdict(lambda: [3, 3, None, False, False])
...
for var,stats in id_to_genotypes.items():
	intersections[(stats[3], stats[4])] += 1

print('in_baseline\tin_callset\n')
for i,j in intersections.items():
	print('\t'.join([str(i[0]), str(i[1]), str(j)]))
	if i[0]:
		check_base += j
	if i[1]:
		check_call += j
...
unmatched = intersections[(False, True)]
print(unmatched, sum(matrix[3]))
assert sum(matrix[3]) >= unmatched


first_column = ['weighted_concordance', 'genotype_concordance_all', 'genotype_concordance_absent', 'genotype_concordance_het', 'genotype_concordance_hom', 'typed', 'total_baseline_variants', 'total_unmatched']
second_column = [str(balanced_concordance), str(overall_concordance), str(conc_absent), str(conc_het), str(conc_hom), str(typed_all), str(all_present), str(unmatched)]
