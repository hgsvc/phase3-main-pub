...
parser.add_argument("--typeclip", default=None)
parser.add_argument(
    "-c",
    "--minclip",
...
for contig, start, end in regions:
    ...
    if contiglen > 0:
        ...
        nf["G"] += cov["G"]
        nf["T"] += cov["T"]
        if args.typeclip == "softread" or "softbase":
            for read in bam.fetch(contig, start, end):
                soft += getSoft(read, group=GROUPS)
        if args.typeclip == "hardread" or "hardbase":
            for read in bam.fetch(contig, start, end):
                hard += getHard(read, group=GROUPS)
        GROUPS += 1
...
soft = pd.DataFrame(
    soft, columns=["contig", "side", "read_count", "value", "position", "read", "group"]
)
...
hard = pd.DataFrame(
    hard, columns=["contig", "side", "read_count", "value", "position", "read", "group"]
)
hard = hard[hard.value >= args.minclip]
hard.sort_values(by=["contig", "position"], inplace=True)
...
for group_id, group in df.groupby(by="group"):
    ...
    if args.typeclip is not None and args.typeclip in [
        "softread",
        "softbase",
        "hardread",
        "hardbase",
    ]:
        if "soft" in args.typeclip:
            tmpsoft = soft[soft.group == group_id]
        if "hard" in args.typeclip:
            tmpsoft = hard[hard.group == group_id]
        if len(tmpsoft) > 0:
            axsoft = ax.twinx()
            axsoft.invert_yaxis()
            bins = args.width * 5
            color = "darkgreen"
            if "read" in args.typeclip:
                weights = tmpsoft.read_count
            if "base" in args.typeclip:
            ...
            if args.typeclip == "softread":
                axsoft.set_ylabel("Soft Clipped Reads", color=color)
            elif args.typeclip == "softbase":
                axsoft.set_ylabel("Soft Clipped Bases (kbp)", color=color)
            elif args.typeclip == "hardread":
                axsoft.set_ylabel("Hard Clipped Reads", color=color)
            elif args.typeclip == "hardbase":
                axsoft.set_ylabel("Hard Clipped Bases (kbp)", color=color)

    if args.coverage is not None:
        print("here")
        ax.axhline(y=int(args.coverage), color="r", linestyle="-")
